...
**Status: Draft v0.8 (evaluation-first correction, 2026-07-10)** — records the implementation work tracked in `docs/tickets/` (T1–T6), which inverted the build priority toward the design's own empirical-tiebreaker ethos: the rent-collection apparatus (§6.6) now exists. Headline: the 30-pair cross-era analog fixture (§6.3) is built and the place-blind structural embedding scores **pair_recall@5 = 0.900, MRR = 0.777** against 30 same-era distractors — the first actual measurement of the system's core bet. **§11.5** records the v0.8 as-built changes; the v0.7 status below is retained for lineage.
...
### 11.5 v0.8 update (2026-07-10, tickets T1–T6 — see `docs/tickets/`)

*The correction this revision records: measurement before machinery. Items below move between the v0.7 tables.*

**Newly implemented (moves from 11.3 "not built" / 11.2 "partial" to built):**

| Design element | Where | Notes |
|---|---|---|
| 30-pair analog fixture + gate (§6.3) | `tests/fixtures/analog_fixture.py` | 30 cross-era pairs (incl. §10.4 non-financial pairs) + 30 distractors. **Baseline: pair_recall@5 = 0.900, MRR = 0.777** (render-v0.8.0 + all-MiniLM-L6-v2). CI-gated at 0.85, ratchet-only. The §3.3 place-blind design goal is now *verified*, not aspirational |
| Masked-ending loop assembled (§6.6) | `evaluation/harness.py` | Snapshot → retrieve → forecast → score → persistence baseline; leakage canaries are hard `LeakageError`s inside the harness, not test conventions. Real-corpus backtests (1907/1929/…) still pending corpus ingestion |
| Composition fixture at tuning scale (§6.6) | `tests/fixtures/composition_fixture.py` | 6 positives / 6 decoys (cross-scope, phase-order, unscoped-singleton decoys). `scripts/tune_thresholds.py` sweeps the episodic gap threshold: both-perfect range 1–8y, so the 2y default is now fixture-justified — resolves half of the 11.4 "untuned thresholds" caution (τ_class remains untuned) |
| Controlled role vocabulary + render scrub (§3.3, §10.1; was 11.2.3) | `extraction/roles.py`, `retrieval/embeddings.py` | ~50-role vocabulary (political-economy + court/succession + Proppian + religious axes), τ_role floor (`NE_TAU_ROLE`, untuned like τ_class), deterministic proper-noun/place/date scrub with permutation-invariance test. Free text can no longer reach the structural embedding. Residue metrics (§10.5 step 1) per source and era in `taxonomy/residue.py` |
| Scope registry (§4; was 11.2.6) | `models.Scope`, `scopes.py`, `data/scope_registry.json` | Versioned registry (scope-v0.1.1, 24 scopes with aliases), exact-alias resolver (no fuzzy matching — wrong-scope poisons the partition; unresolved falls to the visible singleton path). Composition partitions on resolved ids; theses record `scope_registry_version` |
| Embedding/render epoch versioning (§6.3, §11.4 re-embed caution) | `retrieval/epochs.py`, `reembed.py` | Every vector carries its (render, model) epoch; retrieval filters to the current epoch (stale = invisible, not silently wrong); composition treats epoch mismatch as missing signal; idempotent batch re-embed job. The 11.4 re-embed caution is now machinery, not a footgun |
| Analog dedup (§11.4 duplicate-narration caution, partially mitigated) | `retrieval/analog_retrieval.py`, `thesis/generator.py` | SAME_EVENT_AS collapse + conservative interim heuristic (surface-embedding identity signal per §3.3a; near-miss decoys separated by span overlap); per-source cap; theses disclose collapsed narrations. Full mitigation still awaits entity resolution |
| Model↔ORM round-trip gates (§4 schema-first) | `tests/unit/test_roundtrip.py` | Maximal/minimal round-trips for all six persisted aggregates + field-coverage tripwire. Landing this exposed and fixed silent data loss: actors/source_passages never persisted, three Thesis fields with no columns, dropped timestamps, naive/aware datetime drift |

**New standing cautions (v0.8):**

- The analog fixture was authored with LLM assistance and shares the canon's priors (§3.2.1); human review of the 30 pairs is required before the gate is treated as ground truth rather than regression protection.
- The recall baseline was first measured *after* the v0.8 render change, so there is no pre-scrub comparison; the ratchet protects the future, not the past.
- τ_class remains untuned; the analog fixture now makes tuning it possible (§8 step 2), which is the natural next use of the harness.

**Still not built (unchanged from 11.3):** ActorEntity/cohorts, second-pass classification wiring, unclassified-density trigger, promotion loop end-to-end, provisional composition, framework-import flow, taxonomy A/B, mechanism-claim tests, dyadic retrieval, review UI, Prefect orchestration, mechanism vocabulary as data.
