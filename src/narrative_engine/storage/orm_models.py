...
episode_actor_association = Table(
    ...
    Base.metadata,
    MappedColumn("episode_id", UUID(as_uuid=True), ForeignKey("episodes.id", ondelete="CASCADE"), primary_key=True),
    MappedColumn("actor_id", UUID(as_uuid=True), ForeignKey("actors.id", ondelete="CASCADE"), primary_key=True),
)
...
cycle_episode_association = Table(
    ...
    Base.metadata,
    MappedColumn("cycle_id", UUID(as_uuid=True), ForeignKey("cycles.id", ondelete="CASCADE"), primary_key=True),
    MappedColumn("episode_id", UUID(as_uuid=True), ForeignKey("episodes.id", ondelete="CASCADE"), primary_key=True),
)
...
class SourcePassageORM(Base):
    ...
    __table_args__ = (
        Index("ix_source_passages_work_passage", "work_id", "passage_id"),
    )
